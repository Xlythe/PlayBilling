...
/**
    ...
    int getResponseCodeFromBundle(Bundle b) {
        Object o = b == null ? null : b.get(RESPONSE_CODE);
        if(o == null) {
    ...
    void flagStartAsync(String operation) {
        if(mAsyncInProgress) return;
        // if(mAsyncInProgress) throw new
        // IllegalStateException("Can't start async operation (" + operation +
    ...
    int querySkuDetails(String itemType, Inventory inv, List<String> moreSkus) throws RemoteException, JSONException {
        if(mContext == null) return IABHELPER_BAD_RESPONSE;
