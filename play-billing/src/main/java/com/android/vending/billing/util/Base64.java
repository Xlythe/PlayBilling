...
class Base64 {
     ...
     * @return the decoded data
     */
    public static byte[] decodeWebSafe(String s) throws DecoderException {
        byte[] bytes = s.getBytes();
     ...
     */
    public static byte[] decode(byte[] source) throws DecoderException {
     ...
     * @return the decoded data
     */
    public static byte[] decodeWebSafe(byte[] source) throws DecoderException {
     ...
     * @throws DecoderException
     */
    public static byte[] decode(byte[] source, int off, int len) throws DecoderException {
     ...
     * @return decoded data
     */
    public static byte[] decodeWebSafe(byte[] source, int off, int len) throws DecoderException {
     ...
     * @return decoded data
     */
    public static byte[] decode(byte[] source, int off, int len, byte[] decodabet) throws DecoderException {
        int len34 = len * 3 / 4;
        ...
        for(i = 0; i < len; i++) {
            ...
            if(sbiDecode >= WHITE_SPACE_ENC) { // White space Equals sign or
                ...
                if(sbiDecode >= EQUALS_SIGN_ENC) {
                    ...
                    if(sbiCrop == EQUALS_SIGN) {
                        ...
                        if(b4Posn == 0 || b4Posn == 1) {
                            throw new DecoderException("invalid padding byte '=' at byte offset " + i);
                        }
                        else if((b4Posn == 3 && bytesLeft > 2) || (b4Posn == 4 && bytesLeft > 1)) {
                            throw new DecoderException("padding byte '=' falsely signals end of encoded value " + "at offset " + i);
                        }
                ...
                throw new DecoderException("Bad Base64 input character at " + i + ": " + source[i + off] + "(decimal)");
            }
        }
        ...
        if(b4Posn != 0) {
            if(b4Posn == 1) {
                throw new DecoderException("single trailing character at offset " + (len - 1));
            }
